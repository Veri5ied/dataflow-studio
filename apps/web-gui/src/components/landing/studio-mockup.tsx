const sidebarItems = ["workspaces", "users", "query_history", "saved_queries"];
        ...
        <div className="mockup-body">
          <div className="mockup-sidebar">
            ...
            {sidebarItems.map((item, index) => (
              <div
                key={item}
                className={`ms-item${index === 0 ? " active" : ""}`}
              >
          ...
          <div className="mockup-editor">
            ...
            <div className="editor-area">
              <span className="sql-cmt">
                -- Show workspace usage for the last 30 days
              </span>
              <br />
              ...
              <br />
              &nbsp;&nbsp;<span className="sql-fn">COUNT</span>(qh.id){" "}
              <span className="sql-kw">AS</span> query_count,
              <br />
              &nbsp;&nbsp;<span className="sql-fn">COUNT</span>(
              <span className="sql-kw">DISTINCT</span> wm.user_id){" "}
              <span className="sql-kw">AS</span> active_members
              <br />
              <span className="sql-kw">FROM</span>{" "}
              <span className="sql-tbl">workspaces</span> w
              <br />
              <span className="sql-kw">LEFT JOIN</span>{" "}
              <span className="sql-tbl">query_history</span> qh
              <br />
              &nbsp;&nbsp;<span className="sql-kw">ON</span> qh.workspace_id =
              w.id
              <br />
              &nbsp;&nbsp;<span className="sql-kw">AND</span> qh.executed_at
              &gt; <span className="sql-fn">NOW</span>() -{" "}
              <span className="sql-str">INTERVAL '30 days'</span>
              <br />
              <span className="sql-kw">LEFT JOIN</span>{" "}
              <span className="sql-tbl">workspace_members</span> wm{" "}
              <span className="sql-kw">ON</span> wm.workspace_id = w.id
              <br />
              <span className="sql-kw">GROUP BY</span> w.id, w.name
              <br />
              <span className="sql-kw">ORDER BY</span> query_count{" "}
            ...
            <div className="results-table">
              ...
              {rows.map(([name, queryCount, members, status], index) => (
                <div key={name} className="rt-row">
                  ...
                  <span>
                    <span
                      className={`badge-sm ${status === "active" ? "badge-active" : "badge-trial"}`}
                    >
                      {status}
                    </span>
                  </span>
